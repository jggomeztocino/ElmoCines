...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadSessionDates(movie) {
        fetch(`/sessions/${movie}`)
            ...
            .then(sesiones => {
                ...
                const fechas =
                    sesiones.filter(sesion => sesion.fecha >= today && sesion.butacas_libres > 0)
    ...
    function loadSessionTimes(movie, fechaSeleccionada, sesiones) {
        const sesionesDelDia = sesiones.filter(sesion => sesion.fecha === fechaSeleccionada);
        const formulario = document.querySelector('.formulario');
        ...
        sesionesDelDia.forEach((sesion) => {
            if (sesion.butacas_libres > 0) {
                const option = document.createElement('option');
                option.value = sesion.hora;
                option.textContent = sesion.hora;
        ...
        selectorHora.addEventListener('change', (event) => {
            ...
            sesion = fechaSeleccionada.split('-').join('') + '-' + horaSeleccionada.split(':').join('');
            const sesionSeleccionada = sesionesDelDia.find(sesion => sesion.hora === horaSeleccionada);
